...
# ============================================================
# Step 12: Force WiFi to 2.4GHz band
# ============================================================
# The Pi 4's WiFi chip (BCM43455) has weak 5GHz range. If the router
# offers both bands under the same SSID, the Pi may connect to 5GHz
# with poor signal, causing AirPlay dropouts and crashes.
info "Forcing WiFi to 2.4GHz band..."
WIFI_CONN=$(ls /etc/NetworkManager/system-connections/*.nmconnection 2>/dev/null | head -1)
if [ -n "$WIFI_CONN" ]; then
    if ! grep -q "^band=" "$WIFI_CONN" 2>/dev/null; then
        sudo sed -i '/^mode=infrastructure/a band=bg' "$WIFI_CONN"
        info "Set WiFi band to 2.4GHz (bg) in $(basename "$WIFI_CONN")"
    else
        info "WiFi band already configured"
    fi
else
    warn "No NetworkManager WiFi connection found — configure WiFi first"
fi

# ============================================================
# Step 13: Enable overlay filesystem (power-cut protection)
# ============================================================
